...
public class Main {
    ...
    private static byte[] encrypt(byte[] message, byte[] publicKey) {
        byte[] VyBytes = new byte[R_BYTES << 1];
        for (int i = 0; i < R_BYTES << 1; i++) {
            VyBytes[i] = publicKey[i + (R_BYTES << 1)];
        }
        ...
        /* ~DEBUG~ */
        //System.out.println(HEXF.formatHex(Z.y.toByteArray()));
        //byte[] temp = new byte[32];
        //for (int i = 0; i < 32; i++) {
        //    temp[i] = ZyBytes[i + ZyBytes.length - 32];
        //}
        /* ~DEBUG~ */
        byte[] out = new byte[(R_BYTES << 1) * 2 + c.length + t.length];

        for (int i = 0; i < R_BYTES << 1; i++) {
            out[i] = ZxBytes[i];
        }
        for (int i = 0; i < R_BYTES << 1; i++) {
            out[i + (R_BYTES << 1)] = ZyBytes[i];
        }
        for (int i = 0; i < c.length; i++) {
            out[i + (R_BYTES << 1) * 2] = c[i];
        }
        for (int i = 0; i < 32; i++) {
            out[i + (R_BYTES << 1) * 2 + c.length] = t[i];
        }
    ...
    private static byte[] decrypt(byte[] encrypted, String passphrase) {
        ...
        byte[] ZyBytes = new byte[R_BYTES << 1];
        byte[] t = new byte[32];
        byte[] c = new byte[encrypted.length - (R_BYTES << 1) * 2 - 32];
        for (int i = 0; i < ZyBytes.length; i++) {
            ZyBytes[i] = encrypted[i + (R_BYTES << 1)];
        }
        for (int i = 0; i < c.length; i++) {
            c[i] = encrypted[i + (R_BYTES << 1) * 2];
        }
        for (int i = 0; i < t.length; i++) {
            t[i] = encrypted[i + (R_BYTES << 1) * 2 + c.length];
        }
